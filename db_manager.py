...
SQL_SERVER_CONFIG = {
    'server': 'localhost\\SQLEXPRESS', # Target your active named SQLEXPRESS instance
    'database': 'bioscan',             # Database name
    'trusted_connection': 'yes',       # 'yes' for Windows Auth, 'no' for SQL Auth
    'username': '',                    # Required only if trusted_connection is 'no'
    'password': '',                    # Required only if trusted_connection is 'no'
...
class DatabaseManager:
    ...
    def register_or_get_patient(self, name, age, weight, gender):
        ...
        if row:
            ...
            # Patient doesn't exist, create a new profile with random Blood Group
